	...
	SubShader
	{
		...
		Pass
		{
			...
			CBUFFER_START(UnityPerMaterial)
				float4 _Spectra;
				float4 _Center; // Corrected to float4 to match Vector property serialization and ensure correct constant buffer alignment for GPU instancing
				float _RingSrtide;
			...
			half4 frag(Varyings input) : SV_Target
			{
				...
				float3 center = input.positionWS - _Center.xyz;
