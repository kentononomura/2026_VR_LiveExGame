...
public class MirrorReflection : MonoBehaviour
{
	...
	{
		// Subscribe to URP's camera rendering callbacks
		RenderPipelineManager.beginCameraRendering += ExecutePlanarReflection;
	}
	...
	private void CleanUp()
	{
		...
		{
			if (kvp.Value is Camera cam && cam != null)
			{
	...
	// URP callback to handle manual rendering of reflection camera
	private void ExecutePlanarReflection(ScriptableRenderContext context, Camera cam)
	{
		if (!enabled || !GetComponent<Renderer>() || !GetComponent<Renderer>().sharedMaterial || !GetComponent<Renderer>().enabled)
			return;

		// MainCamera と SceneView カメラ以外のカメラ（BackScreenCamera等）は反射の描画をスキップ
		if (cam.cameraType != CameraType.SceneView && !cam.CompareTag("MainCamera"))
			return;

		// Prevent reflection cameras from reflecting themselves recursively
		if (cam.name.Contains("Mirror Refl Camera"))
			return;
		...
		// Check for NaN or Infinity in projection matrix to prevent Unity projection errors (especially in VR/XR)
	...
	private void CreateMirrorObjects(Camera currentCamera, out Camera reflectionCamera)
	{
		...
		if (!m_ReflectionTexture || m_OldReflectionTextureSize != m_TextureSize)
		{
			...
			m_ReflectionTexture.name = "__MirrorReflection" + GetHashCode();
			...
			m_ReflectionDepthTexture.name = "__MirrorReflectionDepth" + GetHashCode();
		...
		{
			GameObject go = new GameObject("Mirror Refl Camera id" + GetHashCode() + " for " + currentCamera.GetHashCode(), typeof(Camera), typeof(Skybox));
	...
	{
		// Safeguard against uninvertible projection matrices (common in XR/VR cameras)
		if (Mathf.Abs(projection.determinant) < 1e-6f)
			return;
		...
		// Safeguard against division by zero which creates Infinities/NaNs
